...
namespace RPG
{
    ...
    {
        public TileCell[,] tileMap;

        public TileMap(Vector2 pos,int width,int height)
        {
            Position = pos;
            tileMap = new TileCell[width, height];
            Width = width;
            Height = height;
            Initialize();
        }
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            for (int x = 0; x < Width; x++)
            {
                ...
                for (int y = 0; y < Height; y++)
                {
                    ...
                    {
                        DrawTile(tileMap[x, y].tile, pos, Layers.TILES, spriteBatch);
                    }
                    ...
                    {
                        DrawTile(tile, pos, Layers.TILES + 0.000001f * mergeTilesDepth, spriteBatch);
                        mergeTilesDepth++;
                    }
                    ...
                    {
                        DrawTile(tile, pos - new Vector2(0, TileSet.tileHeight * topTilesCount), Layers.TOPTILES + 0.000001f * y, spriteBatch);
                        topTilesCount++;
                    }
        ...
        private bool CheckIsTileInView(float x, float y)
        {
            ...
            bool right = x < Main.camera.Position.X + Main.WindowWidth;
            bool down = y < Main.camera.Position.Y + Main.WindowHeight;
            return left && top && right && down;
        }

        private void DrawTile(Tile tile,Vector2 pos,float depth, SpriteBatch spriteBatch)
        {
            if (CheckIsTileInView(pos.X, pos.Y))
            {
                spriteBatch.Draw(TileSet.SpriteSheet[tile.tileSet], pos, TileSet.GetSourceRectangle(tile), Color.White, 0, new Vector2(), 1f, SpriteEffects.None, depth);
            }
        }
