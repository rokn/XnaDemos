...
namespace TopScrollingGame
{
    ...
    public class Animation
    {
        ...
        public void Draw(SpriteBatch spriteBatch, float depth, Color color, Rectangle rect)
        {
            if (isSpriteSheet)
            {
                spriteBatch.Draw(sourceSpriteSheet, rect, sourceRectangles[Index], color, rotation, new Vector2(), SpriteEffects.None, depth);
            }
            else
            {
                spriteBatch.Draw(currentTexture, rect, null, color, rotation, new Vector2(), SpriteEffects.None, depth);
            }
        }
    }
}