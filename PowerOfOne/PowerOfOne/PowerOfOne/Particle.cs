...
namespace PowerOfOne
{
    public class Particle
    {
        public Particle(Texture2D texture, Vector2 position, Vector2 velocity,
            float angle, float angularVelocity, Color fromColor, Color toColor, float startingSize, float endSize, int ttl)
        {
            Texture = texture;
            Position = position;
            Velocity = velocity;
        ...
        {
            spriteBatch.Draw(Texture, Position, null, RealColor, Angle, Origin, Size, SpriteEffects.None, 0.7f);
        }
    }
}